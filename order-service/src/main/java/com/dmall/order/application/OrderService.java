...
public class OrderService {
  ...
  public void notifyInDelivery(Integer oid, String shipping_id, String shipping_time) {
    ...
    Message<OrderEvents> message = getOrderEventsMessage(shipping_id, shipping_time);

    final OrderEntity order = orderEntityFactory.build(oid);
    order.sendEvent(message);
  }

  private Message<OrderEvents> getOrderEventsMessage(String shipping_id, String shipping_time) {
    final OrderEvents orderShipped = OrderEvents.OrderShipped;
    return MessageBuilder
          .withPayload(orderShipped)
          .setHeader("shipping_id", shipping_id)
          .setHeader("shipping_time", shipping_time)
        .setHeader("class", String.class.getName())
          .build();
  }
